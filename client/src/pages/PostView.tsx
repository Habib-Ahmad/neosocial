    ...
    <>
        ...
        <Card className="backdrop-blur-sm bg-white/80 border-purple-100 shadow-lg">
          <CardHeader className="pb-3">
            <div className="flex items-center space-x-3">
              <img
                src={
                  post.author.profile_picture.includes("http")
                    ? post.author.profile_picture
                    : `${IMG_BASE_URL}${post.author.profile_picture}`
                }
            ...
            {post.media_urls?.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-2">
                {post.media_urls.map((url, index) => (
                  <img
                    key={index}
                    src={url.includes("http") ? url : `${IMG_BASE_URL}${url}`}
      ...
      {activeImageIndex !== null && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-80 flex items-center justify-center">
          ...
          <img
            src={`http://localhost:5000${post.media_urls[activeImageIndex]}`}
